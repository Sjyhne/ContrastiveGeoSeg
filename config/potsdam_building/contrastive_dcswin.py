...
from geoseg.datasets.potsdam_building_dataset import *
from geoseg.models.ContrastiveDCSwin import ContrastiveDCSwin
...
model = ContrastiveDCSwin(config=config, attn_loss=attn_loss, contrastive_loss=con_loss)
...
# weights and biases
wandb_logging_name = f"SupCon"
wandb_project = f"PotsdamBuilding ContrastiveCE {dp}"
wandblogger = True
...
train_dataset = PotsdamDataset(
    data_root='data/potsdam_building/train', 
    mode='train',
    mosaic_ratio=0.25,
    transform=train_aug,
    data_percentage=dp
...
val_dataset = PotsdamDataset(transform=val_aug, data_percentage=dp)
