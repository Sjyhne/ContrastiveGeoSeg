...
from .sub_models.dat_blocks import *
...
class DAT(nn.Module):
    ...
    def __init__(self, img_size=224, patch_size=4, num_classes=1000, expansion=4,
                 ...
                 stage_spec=[['L', 'D'], ['L', 'D'], ['L', 'D', 'L', 'D', 'L', 'D', 'L', 'D'], ['L', 'D']], 
                 groups=[1, 1, 2, 4],
        ...
        for i in range(4):
            self.down_projs.append(
                nn.Sequential(
                    nn.Conv2d(dims[i], dims[i], 1, 1, 0, bias=False),
                    LayerNormProxy(dims[i])
                ) if use_conv_patches else nn.Sequential(
                    nn.Conv2d(dims[i], dims[i], 2, 2, 0, bias=False),
                    LayerNormProxy(dims[i])
                )
            )
